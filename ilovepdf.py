...
if choice == "PDF分割 (Split)":
    ...
    if uploaded_file:
        ...
        with col_view:
            st.subheader("プレビュー")
            # scroll_to_page等のパラメータは指定せず、heightを確保することでスクロール表示を促します
            pdf_viewer(uploaded_file.getvalue(), height=800, width=None)
    ...
    if uploaded_files:
        ...
        with col_view:
            ...
            if target_file:
                pdf_viewer(target_file.getvalue(), height=600)
        ...
        with col_view:
            ...
            if st.checkbox("各ページのサムネイルを表示する", value=True):
                try:
                    ...
                    st.warning("Windowsの場合はPopplerがインストールされていないとサムネイルが表示できません。ページ番号だけで操作してください。")
